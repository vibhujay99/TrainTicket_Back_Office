...
function TrainCaller(props) {
	const [sold, setSold] = useState([]);
	const [booked, setBooked] = useState([]);
	const [numberOfSeats, setNumberOfSeats] = useState(0);
	const [error, setError] = useState('');
	const [details, setDetails] = useState({ name: 'Train' });
	...
	const fetchAllBookingDetails = async () => {
		const resp = await getTrainBySlug(props.match.params.slug).catch(err => {
...
export default withRouter(TrainCaller);
