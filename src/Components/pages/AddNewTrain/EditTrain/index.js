...
import { getTrainBySlug } from "../../../../Utils/Requests/Train";
...
class Edittrain extends Component {
  ...
  fetchLocations = async () => {
    ...
    if (resp.status === 200) {
      this.setState({
        locations: resp.data,
        startLocation: this.state.train.startLocation,
        endLocation: this.state.train.endLocation
      });
    }
    this.fetchTravels();	
  };
  ...
  fetchTrainDetails = async () => {
    const { slug } = this.props.match.params;

    const resp = await getTrainBySlug(slug).catch(err => {
    ...
    if (resp && resp.status === 200) {
      this.setState({
        ...
        name: resp.data.name,
        type: resp.data.type,
        trainNumber: resp.data.trainNumber,
        fare: resp.data.fare,
